 ...
 * \param network
 */
void OSPF_BGP(Graph& network, vector<Commodity*>& v_commodity)
{
	DijkstraShortestPathAlg shortest_path_alg(&network);
	double Total_Thr = 0;
	double max_cost = 0;
	for (vector<Commodity*>::iterator it = v_commodity.begin(); it != v_commodity.end(); ++it)
	{
		int source_node_id = (*it)->source_->getID() + (*it)->source_->getGraphID() * max_Nodes;
		int sink_node_id = (*it)->sink_->getID() + (*it)->sink_->getGraphID() * max_Nodes;

		BasePath* result =
		    shortest_path_alg.get_shortest_path(network.get_vertex(network.mpNodeID.at(source_node_id),network.get_graphID()),
		    		network.get_vertex(network.mpNodeID.at(sink_node_id),network.get_graphID()));
        if (result->length() > 0)
        {
    		...
    		{
    			network.set_edge_BW(result->GetVertex(i),result->GetVertex(i+1),
    					network.get_edge_BW(result->GetVertex(i),result->GetVertex(i+1))-Thr);
    		}
...
void BCP_TE(vector<Graph*>& ASes, vector<InterGraph*>& InterAS)
{
	/*
...
int main(...)
{
	...
	for (vector<Graph*>::iterator it = ASes.begin(); it != ASes.end(); ++it)
	{
		...
		(*it)->ConstructVirtualGraph_v1(temp,file_name);
