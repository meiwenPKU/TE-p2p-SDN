 ...
 * In this naive protocol, all k-best routes info are advertised to the
neighbors
 * And we will evaulate the computation overhead, msg overhead, and memory
overhead
...
void naive_TE(vector<Graph *> &ASes, vector<InterGraph *> &InterAS) {
  ...
  for (timeStamp = 0; timeStamp < 1000000; timeStamp++) {
            ...
            for (vector<Graph *>::iterator it_AS = ASes.begin(); it_AS != ASes.end(); ++it_AS) {
              ...
              if ((*it_AS)->get_graphID() == neighborASid) {
                for (set<BaseVertex *>::iterator it_peer = vertex_set.begin();
                     it_peer != vertex_set.end(); ++it_peer) // the peer switch of the border switch
                {
                  for (vector<TopoTableEntry>::iterator it_topoEntry =
                           (*it_AS)->m_AdvertisedTopoTable.m_vEntry.begin();
                       it_topoEntry != (*it_AS)->m_AdvertisedTopoTable.m_vEntry.end();
                       ++it_topoEntry) {
    ...
    //   (*it)->printTopoTable();
    //   cout << "The advertise table of AS " << (*it)->get_graphID() << endl;
...
int main(int argc, char **argv) {
  ...
  for (vector<Graph *>::iterator itl = ASes.begin(); itl != ASes.end(); ++itl) {
    for (vector<Graph *>::iterator itr = ASes.begin(); itr != ASes.end();
         ++itr) {
   ...
   * 1. for every vertex in the topology table but not in the AS, we add a new
   * edge from its peer to it
   * 2. the weight and bandwidth of the new edge equal to the corresponding item
  ...
  for (time = 0; time < runTime; time++) {
    // cout << "size of v_Allocation = " << sizeof(map<Commodity*,
    // set<pair<BasePath,double>,setcomp> >) * v_Allocation.size() << endl;
    v_Allocation.clear();
    ...
    // get the memory footprint
    // process_mem_usage(vm, rss);
    // cout << "After generating commodities, VM: " << vm << "; RSS: " << rss <<
    // endl;

    cout << "time = " << time << endl;
    // each AS do the traffic engineering for its own commodities based on the
    // topology
    ...
    for (vector<Graph *>::iterator it = ASes.begin(); it != ASes.end(); ++it) {
      ...
      for (map<int, double>::iterator it_used =
               VirtualAS[index]->m_mpEdgeCodeUsedBW.begin();
      ...
      for (vector<Commodity *>::iterator it_commodity =
               (*it)->m_vCommodity.begin();
        ...
        if (VirtualAS[index]->mpNodeID.find(source_id) ==
            VirtualAS[index]->mpNodeID.end()) {
          cout << "wrong in MainP.cpp: the source node is not find" << endl;
          exit(EXIT_FAILURE);
        }
        if (VirtualAS[index]->mpNodeID.find(sink_id) ==
            VirtualAS[index]->mpNodeID.end()) {
          // cout << "Commodity (" << (*it_commodity)->source_->getGraphID() <<
          // "," << (*it_commodity)->source_->getID() << ") --> (" <<
          // (*it_commodity)->sink_->getGraphID() << "," <<
          // (*it_commodity)->sink_->getID() << ") is not reachable" << endl;
          continue;
        }
        ...
        BaseVertex *p_source = VirtualAS[index]->get_vertex(
            s_vertexID, VirtualAS[index]->get_graphID());
        BaseVertex *p_sink = VirtualAS[index]->get_vertex(
            d_vertexID, VirtualAS[index]->get_graphID());
        Commodity *map_com =
            new Commodity(p_source, p_sink, (*it_commodity)->demand_);
        Allocation[map_com] = empty_set;
        VirtualAS[index]->m_vCommodity.push_back(map_com);
      }
      // cout << "The total send feasible throughput = " << totalSendThr <<
      // endl; Google_TE_Optimization(VirtualAS[index], &Allocation);
      // get the memory footprint
      // process_mem_usage(vm, rss);
       ...
       */
      for (auto it_com = Allocation.begin(); it_com != Allocation.end();
           ++it_com) {
        // it_com->first->Print(result);
        double totalAllocated = 0;
        for (set<pair<BasePath, double>, setcomp>::iterator it_set =
                 it_com->second.begin();
    ...
    vector<vector<Commodity *>> NewCommodity(ASes.size(),
                                             vector<Commodity *>());
    ...
    for (auto it = v_Allocation.begin(); it != v_Allocation.end(); ++it) {
      ...
      /*
       * if the commodity's destination and source are in the same AS, then the
       * commodity arrives the destination. Otherwise, the commodity enter into
       * transit AS
       */
      for (it_map = (*it).begin(); it_map != (*it).end(); ++it_map) {
        ...
        VirtualAS[index]->get_original_id(it_map->first->source_->getID(),
                                          oriNodeID, oriASID);
        // cout << "(" << oriASID << "," << oriNodeID << ")-->";
        VirtualAS[index]->get_original_id(it_map->first->sink_->getID(),
                                          oriNodeID, oriASID);
        // cout << "(" << oriASID << "," << oriNodeID << ");";
        // cout << "demand = " << it_map->first->demand_ << "; Allocated =" <<
        // it_map->first->Allocated_ << endl;

        BaseVertex *MappedNode =
            NetworkToAS(*(VirtualAS[index]), ASes, it_map->first->sink_);
        if (MappedNode->getGraphID() == ASes[index]->get_graphID()) {
          ...
          for (set<pair<BasePath, double>, setcomp>::iterator it_set =
                   it_map->second.begin();
               it_set != it_map->second.end(); ++it_set) {
            // find the first switch along the path which does not belong to the
            // AS
            BaseVertex *p_border;
            // cout << "allocation to this path = " << it_set->second << endl;
            VirtualAS[index]->get_original_id(
                it_set->first.GetVertex(0)->getID(), oriNodeID, oriASID);
            // cout << "(" << oriASID << "," << oriNodeID << ")";
            int len = it_set->first.length();
            for (int i = 1; i < len; ++i) {
              BaseVertex *Pref_MappedNode = NetworkToAS(
                  *(VirtualAS[index]), ASes, it_set->first.GetVertex(i - 1));
              BaseVertex *MappedNode = NetworkToAS(*(VirtualAS[index]), ASes,
                                                   it_set->first.GetVertex(i));

              VirtualAS[index]->get_original_id(
                  it_set->first.GetVertex(i)->getID(), oriNodeID, oriASID);
              // cout << "-->(" << oriASID << "," << oriNodeID << ")";

              Aver_cost += VirtualAS[index]->get_edge_weight(
                               it_set->first.GetVertex(i - 1),
                               it_set->first.GetVertex(i)) *
                           it_set->second;
              if (MappedNode->getGraphID() != ASes[index]->get_graphID()) {
                p_border = MappedNode;
            ...
            // cout << endl;
            if (p_border == NetworkToAS(*(VirtualAS[index]), ASes,
                                        it_set->first.GetLastVertex())) {
              // the commodity arrives at the destination
              Throughput += it_set->second;
            } else {
              // generate the commodity in the neighboring AS
              NewCommodity[p_border->getGraphID()].push_back(
                  new Commodity(p_border,
                                NetworkToAS(*(VirtualAS[index]), ASes,
                                            it_set->first.GetLastVertex()),
                                it_set->second));
            }
          }
        }
        // cout << "Throughput=" << Throughput << "; Aver_cost=" << Aver_cost <<
        // endl; cout << "---------------------------------------" << endl;
      }
    }
    cout << "Throughput=" << Throughput << "; Aver_cost=" << Aver_cost
         << "; numPaths = " << numPaths << endl;
    // get the memory footprint
    // process_mem_usage(vm, rss);
    // cout << "After updating commodities, VM: " << vm << "; RSS: " << rss <<
    // endl;
